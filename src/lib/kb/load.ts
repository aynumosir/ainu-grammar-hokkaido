 ...
 * binding does not exist and a plain fetch serves static/ as it is.
 */
...
export async function kbJson<T>(
	...
	const res = assets ? await assets.fetch(new Request(target.toString())) : await fetchFn(target.toString());
